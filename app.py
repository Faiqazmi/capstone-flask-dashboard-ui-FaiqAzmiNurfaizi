        ...
        'rev_table' : df2.groupby(['Category','App']).agg(Reviews=('Reviews', np.sum), 
                                    Rating=('Rating', np.mean)).\
                                            reset_index().sort_values('Reviews', 
                                                                      ascending=False).head(10).reset_index(drop=True).astype({'Reviews':float}).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }

    ## Bar Plot
    cat_order = cat_order = df2.groupby('Category').agg({'Category' : 'count'}).\
